...
for ch=1:9
    for b=1:64
        for n=1:length(w(b,:))
            h_0(b,n,ch)=h_sub_64_L(b,n,ch)*(1-w(b,n));
            h_1(b,n,ch)=h_sub_64_R(b,n,ch)*(1-w(b,n));
        end
    end
end
...
T_m_plf=5;
L_avg=N_MT_av__64(:,T_m_plf);

for b=1 : 64
    L_proc(b)=(L_avg(b)/L_voff(b)+0.5)*L_voff(b);
end



for b=1:64
    L_plf(b)=L_proc(b)-L_voff(b)+N_fft(b)/2;
end


for b=1:64
    N_L_blk(b)=max([2*L_plf(b)/N_fft(b) 1]);
end

N_L_blk=ceil(N_L_blk);



X_DMX=X_sub;

% Late reverberation subblock
for ch=1:9
for b=1:64
    for s=1:N_L_blk(b)
         s_hat=N_DE_blk(b)-1+s;
        for n=1:N_fft(b)/2  
            h_sub_0(n,s,b,ch)=h_0(b,n+(s_hat)*N_fft(b)/2,ch);
            h_sub_1(n,s,b,ch)=h_1(b,n+(s_hat)*N_fft(b)/2,ch);
        end
    end
end
end
...
for ch=1:9
    for b=1:64
        for s=1:length(h_sub_0(1,:,1,1))
            H_L_0(:,s,b,ch) = fft(h_sub_0(:,s,b,ch),N_fft(b));
            H_L_1(:,s,b,ch) = fft(h_sub_1(:,s,b,ch),N_fft(b));
        end
    end
end

%% match Energy Decay
H_ED_0=zeros(size(H_L_0(:,:,:,1)));
H_ED_1=zeros(size(H_L_1(:,:,:,1)));

for ch=1:9
    for b=1:64
        for s=1:length(H_L_0(k,:,b,ch))
            for k=1:N_fft(b)
                H_ED_0(k,s,b)=H_ED_0(k,s,b)+H_L_0(k,s,b,ch);
                H_ED_1(k,s,b)=H_ED_1(k,s,b)+H_L_1(k,s,b,ch);
            end
        end
end
end

H_ED=(H_ED_0+H_ED_1)/(2*channel);



%% fast convolution

Y_L_EDM=zeros(length(X_DMX(:,1,1)),length(X_DMX(1,:,1)),length(X_DMX(1,1,:)));
for b= 1 : length(X_DMX(1,1,:))
    for k=1:length(X_DMX(:,1,1))
        vec=zeros(1,length(H_ED(1,:,1)));
        for r =1 : length(X_DMX(1,:,1))
            vec=[X_DMX(k,r,b), vec(1:end-1)];
            Y_L_EDM(k,r,b) = vec * transpose(H_ED(k,:,b));
        end
    end
end

%% FDIC
sum=zeros(length(H_L_0(:,1,1,1)),length(H_L_0(1,1,:,1)),9);
H_hat_L_0=zeros(size(sum));
H_hat_L_1=zeros(size(sum));
N_L_blk=ceil(N_L_blk);
for ch=1:9
    for b=1:64
          for k=1:N_fft(b)
             for s=1:N_L_blk(b)
                sum(k,b,ch)=sum(k,b,ch)+H_L_0(k,s,b,ch)*conj(H_L_1(k,s,b,ch)) ;
                H_hat_L_0(k,b,ch)=H_hat_L_0(k,b,ch)+abs(H_L_0(k,s,b,ch))^2;
                H_hat_L_1(k,b,ch)=H_hat_L_1(k,b,ch)+abs(H_L_1(k,s,b,ch))^2;
             end
            phi_ch(k,b,ch)=real(sum(k,b,ch))/sqrt(H_hat_L_0(k,b,ch)*H_hat_L_1(k,b,ch));
          end
    end
end

phi=zeros(size(phi_ch(:,:,1)));
for ch=1:9
    for b=1:64
        for k=1:N_fft(b)
            phi(k,b)=phi(k,b)+phi_ch(k,b,ch);
        end
    end
end
...
for b=1:64
    for r=length(Y_L_EDM(1,:,1))
        for k=1;N_fft(b)
            cos_alpha=sqrt((1+phi(k,b))/2);
            sin_alpha=sqrt((1-phi(k,b))/2);
            Y_L_0(k,r,b)=cos_alpha*Y_L_EDM(k,r,b)+sin_alpha*Y_L_EDM(k,r,b);
            Y_L_1(k,r,b)=cos_alpha*Y_L_EDM(k,r,b)+(-sin_alpha)*Y_L_D_EDM(k,r,b);
        end
    end
end
